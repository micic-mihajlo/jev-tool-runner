# Legacy investigation hooks

These hooks run a separate read-only investigation. They do not replace the coding model's tool decision. Use the [native provider router](DAILY-USE.md) for the current architecture. The instructions below are retained for existing installations and removal.
